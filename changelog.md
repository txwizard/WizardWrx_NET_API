...
#### **Changed**
- `DigestAlgorithm` enumeration moved from `DigestFile` into the `WizardWrx.Cryptography` namespace.
  - This reflects its expanded role across the digest subsystem and removes inappropriate coupling to `DigestFile`.
  - Existing callers must update references from `DigestFile.DigestAlgorithm` to `WizardWrx.Cryptography.DigestAlgorithm`.

- Digest formatting logic (`FormatDigestPerOutputFormat`) relocated from `DigestFile` into a new `DigestFormatters` class.
  - This formalizes digest formatting as a distinct concern and improves subsystem cohesion.

#### **Added**
- Comprehensive test suite for digest engines and digest comparison, including SHA‑256, SHA‑384, and SHA‑512 coverage.
  - The digest engine is algorithm‑agnostic; tests confirm correct behavior across all supported algorithms.
  - Empty‑input and nonexistent‑file conditions are now fully validated. Either condition throws a `FileNotFoundException` with a descriptive message.

#### **Impact**
These changes update the public API surface but do not alter digest semantics. Applications using `FormatDigestPerOutputFormat`and its `FormatDigestPerOutputFormat` enumeration will require minor type‑location updates. Otherwise, the existing methods retain their previous locations and signatures.

#### **Rationale**
The digest subsystem has grown beyond its original scope. Centralizing algorithm enumeration and formatting logic improves cohesion, reduces duplication, and aligns with the architectural separation of concerns established across WizardWrx libraries.

---

### **WizardWrx.Common [9.0.291]**

#### **Added**
New symbolic constants added to `MagicNumbers`:

- `BITS_PER_BYTE` — fills a longstanding gap in both WizardWrx and the .NET Base Class Library.
- `BUFFER_SIZE_8KB` — standard buffered I/O size, historically aligned with disk‑cluster and network‑packet conventions.
- `BUFFER_SIZE_80KB` — default streaming buffer size for large‑file operations.
- `CAPACITY_ONE_MEGABYTE` — canonical 1 MB threshold (1,048,576 bytes), used for gulp‑vs‑stream decisions.

These constants support the new file‑comparison subsystem and improve clarity and consistency across I/O‑related code.
...
#### **Added**
A new static `FileComparer` class providing strict, byte‑for‑byte file comparison:

- Automatically selects between `File.ReadAllBytes` and streaming comparison based on file size.
- Uses configurable thresholds (`ReadAllBytesCeiling`) and buffer sizes (`StreamingBufferSize`).
- Provides both strict (`CompareFiles`) and safe (`TryCompareFiles`) comparison APIs.
- Includes a robust `CompareByteArrays` method with strict validation semantics.

This class replaces ad‑hoc comparison logic and provides a unified, scalable mechanism for file equality checks.

#### **Validated**
A full test suite (34 tests) now exercises:

- small‑file and large‑file comparison  
- streaming vs. gulping thresholds  
- digest generation and digest comparison  
- SHA‑256, SHA‑384, and SHA‑512 behavior  
- empty‑file and nonexistent‑file exception semantics  
- cross‑validation between `FileComparer` and `DigestComparer`  

All tests passed successfully.

---

### **WizardWrx.JSONSupport [9.0.56]**

#### **Fixed**
- `JSON_Deserialized_Object` was moved from the incorrect namespace `WizardWrx.JsonSupport` to the correct `WizardWrx.JSONSupport`, matching the project’s `RootNamespace` and `AssemblyName`.

---

### **Summary**

This release delivers a cohesive modernization of the digest subsystem, introduces a robust and scalable file‑comparison engine, expands the `MagicNumbers` library with essential I/O constants, corrects namespace alignment in JSONSupport, and validates all changes with a comprehensive 34‑test suite. The result is a cleaner, more coherent architecture across WizardWrx libraries with improved reliability, clarity, and maintainability.
...
In the `ApplyHeaders` method on class `RequestEngine`,  parameters `memberName`,
`sourceFilePath`, and `sourceLineNumber` woere not appropriately decorated, so
the compiler didn't automatically fill them with the caller's member name,
source file path, and source line number, respectively. This oversight has been
corrected by adding the appropriate attributes to each parameter, ensuring that
they are automatically populated with the correct values when the method is
called. This enhancement allows for better logging and debugging capabilities,
as it provides contextual information about where the headers were applied in
the code. 

This build also resolves four warnings that were generated by the compiler due
to the missing attributes, improving code quality and maintainability.
