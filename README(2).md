...
__AssemblyFileVersion__ was intended to tie together all the assemblies in this
API. Howwever, due to a misconfiguration that just came to light as version 7.12
came together, `AssemblyFileVersion` was missing, while the solution-wide
AssemblyInfo.cs that whould have stored it had instead the `AssemblyVersion` key.
